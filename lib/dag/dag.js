...
const { pipelineKind, nodeKind } = require('@hkube/consts');
...
class DAG extends EventEmitter {
    ...
    _buildGraph(pipeline, options) {
        ...
        this._validateOutputNodes(pipeline);
    ...
    _validateOutputNodes(pipeline) {
        const outputs = this.getAllNodes().filter(n => n.kind === nodeKind.Output);
        if (outputs.length > 0 && pipeline.kind === pipelineKind.Stream) {
            throw new Error(`node "${outputs[0].nodeName}" from kind output can not be used in a streaming pipeline`);
        }
        ...
        if (withChilds.length) {
            throw new Error(`node "${withChilds[0].nodeName}" should not have child nodes`);
        }
        if (noParents.length) {
            throw new Error(`node "${noParents[0].nodeName}" should have parent nodes`);
        }
    }
