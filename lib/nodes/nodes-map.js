...
class NodesMap extends EventEmitter {
    ...
    _buildGraph(pipeline, options) {
        ...
        nodes.forEach((n) => {
            ...
            if (!n.algorithmName && !n.pipelineName) {
                throw new Error('please provide algorithmName or pipelineName');
            }
            ...
            n.input.forEach((i) => {
                ...
                results.forEach((r) => {
                    ...
                    if (!edge) {
                        this.setEdge(source, target, { types: [r.type] });
                    }
                    else {
        ...
        const statelessNodes = this._graph.sources().map(s => this._graph.node(s)).filter(s => !s.stateType || s.stateType === stateType.Stateless);
