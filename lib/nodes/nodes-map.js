...
class NodesMap extends EventEmitter {
  ...
  _buildGraph(options) {
    ...
    nodes.forEach(n => {
      n.input.forEach(i => {
        ...
        results.forEach(r => {
          let node = nodeList.find(
            f => f.source === r.nodeName && f.target === n.nodeName
          );
          if (!node) {
            node = {
              source: r.nodeName,
  ...
  _checkChildNode(source, target, ind) {
    ...
    if (
      this._isWaitAny(edges) &&
      (this._isWaitNode(edges) || this._isWaitBatch(edges))
    ) {
      index = null;
  ...
  _isWaitAny(edges) {
    return (
      edges.includes(consts.relations.WAIT_ANY) ||
      edges.includes(consts.relations.WAIT_ANY_BATCH)
    );
  }
  ...
  updateCompletedTask(task) {
    ...
    if (childs) {
      return childs.map(child =>
        this._checkChildNode(task.nodeName, child, task.batchIndex)
      );
    }
    return null;
  }
  ...
  updateAlgorithmExecution(options) {
    ...
    if (!execNode) {
      ...
      this._graph.setEdge(nodeName, uniqueNodeName, [
    ...
    if (!execBatch) {
      execBatch = new ExecBatch({
        ...execNodeParams,
        batchIndex: execNode.batch.length + 1
      });
