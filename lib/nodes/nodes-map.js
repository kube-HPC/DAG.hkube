...
class NodesMap extends EventEmitter {
    ...
    _buildGraph(options) {
        ...
        nodes.forEach((n) => {
            n.input.forEach((i) => {
                ...
                results.forEach((r) => {
                    ...
                    if (!node) {
                        node = { source: r.nodeName, target: n.nodeName, types: [r.type] };
                        nodeList.push(node);
                    }
                    else {
                        node.types.push(r.type);
                    }
                });
            });
        ...
        nodeList.forEach((n) => {
            ...
            if (source) {
                this.setEdge(n.source, n.target, { types: n.types });
            }
        });
        edges.forEach((e) => {
            const edge = this.getEdge(e.source, e.target);
            if (!edge) {
                this.setEdge(e.source, e.target, { types: [consts.relations.WAIT_NODE] });
            }
        });
        this._addLevels();
    }
