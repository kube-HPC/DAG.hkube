...
class NodesMap extends EventEmitter {
    ...
    _buildGraph(options) {
        ...
        nodeList.forEach((n) => {
            ...
            if (source) {
                this.setEdge(n.source, n.target, { edges: n.edges });
            }
        });
        edges.forEach((e) => {
            const edge = this.getEdgeTypes(e.source, e.target);
            if (!edge) {
                this.setEdge(e.source, e.target, { edges: [{ type: 'waitNode' }] });
            }
        });
        this._addLevels();
    }
    ...
    updateAlgorithmExecution(options) {
        ...
        if (!execNode) {
            ...
            this.setEdge(parentNodeName, nodeName, { edges: [{ type: consts.relations.ALGORITHM_EXECUTION }] });
        }
    ...
    updateEdge(source, target, newValue) {
        const oldValue = this.getEdge(source, target);
        this.setEdge(source, target, { ...oldValue, ...newValue });
    }
