...
class NodesMap extends EventEmitter {
    constructor(options) {
        super();
        // for supporting local-pipeline driver
        if (options.parser) {
    ...
    _buildGraph(options) {
        ...
        nodeList.forEach((n) => {
            ...
            if (source) {
                this._graph.setEdge(n.source, n.target, { edges: n.edges });
            }
        });
        edges.forEach((e) => {
            const edge = this.getEdgeTypes(e.source, e.target);
            if (!edge) {
                this._graph.setEdge(e.source, e.target, { edges: [{ type: 'waitNode' }] });
            }
        });
        this._addLevels();
    }
    ...
    _addLevels() {
        const traverse = (node, level) => {
            const targets = this._childs(node);
            const realNode = this.getNode(node);
            realNode.level = Math.max(realNode.level, level); // eslint-disable-line
        ...
        this._graph.sources().forEach(s => traverse(s, 0));
    }
    ...
    updateAlgorithmExecution(options) {
        ...
        if (!execNode) {
            ...
            this._graph.setEdge(parentNodeName, nodeName, { edges: [{ type: consts.relations.ALGORITHM_EXECUTION }] });
        }
    ...
    setEdge(source, target, newValue) {
        const oldValue = this.getEdge(source, target);
        this._graph.setEdge(source, target, { ...oldValue, ...newValue });
    }

    getEdgeTypes(source, target) {
        const edge = this._graph.edge(source, target);
        return edge.edges.map(e => e.type);
    }
