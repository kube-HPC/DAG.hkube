...
class NodesMap extends EventEmitter {
    ...
    _buildGraph(options) {
        ...
        edges.forEach((e) => {
            const edge = this._graph.edge(e.source, e.target);
            if (!edge) {
                this._graph.setEdge(e.source, e.target, [{ type: 'waitNode' }]);
            }
        });
        this._addLevels();
    }
            ...
            if (this._isWaitAny(edges)) {
                if (node.batch.length > 0) {
                    node.batch.forEach((b) => {
                        if (!index || index === b.batchIndex || b.shouldRun) {
    ...
    updateAlgorithmExecution(options) {
        ...
        if (!execNode) {
            ...
            this._graph.setEdge(parentNodeName, nodeName, [{ type: consts.relations.ALGORITHM_EXECUTION }]);
        }
    ...
    isAllParentsFinishedIndex(nodeName, index) {
        ...
        parents.forEach((p) => {
            const node = this.getNode(p);
            const edges = this._graph.edge(p, nodeName).map(e => e.type);
            if (this._isWaitAny(edges)) {
                const batch = node.batch.find(b => b.batchIndex === index);
