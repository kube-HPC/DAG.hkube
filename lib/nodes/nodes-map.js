...
class NodesMap extends EventEmitter {
    ...
    updateAlgorithmExecution(options) {
        const { nodeName, execId, execNodeName, status, error, result } = options;
        const node = this.getNode(nodeName);
        if (!node) {
            throw new Error(`unable to find node ${nodeName}`);
        }
        const execNodeParams = { ...options, nodeName: execNodeName, level: node.level + 1 };
        let execNode = this.getNode(execNodeName);
        if (!execNode) {
            execNode = new ExecNode(execNodeParams);
            this._graph.setNode(execNodeName, execNode);
            this._graph.setEdge(nodeName, execNodeName, [{ type: 'algorithmExecution' }]);
        }
